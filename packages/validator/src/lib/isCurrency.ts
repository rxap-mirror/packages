...
function currencyRegex(options: Required<IsCurrencyOptions>) {
...
export function isCurrency(str: unknown, options: IsCurrencyOptions = {}) {
  assertString(str);
  const _options: Required<IsCurrencyOptions> = merge(options, default_currency_options) as any;
  return currencyRegex(_options).test(str);
}
