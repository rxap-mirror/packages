...
const validators: Record<string, (str: string) => boolean> = {
  ES: (str) => {
    assertString(str);

    const DNI = /^[0-9X-Z][0-9]{7}[TRWAGMYFPDXBNJZSQVHLCKE]$/;

    const charsValue: Record<string, number> = {
      X: 0,
      Y: 1,
      Z: 2,
    };
    ...
    const number = Number(sanitized.slice(0, -1).replace(/[X,Y,Z]/g, char => charsValue[char].toFixed(0)));
  ...
  'zh-TW': (str) => {
    const ALPHABET_CODES: Record<string, number> = {
      A: 10,
      B: 11,
      C: 12,
      D: 13,
    ...
    const sanitized: string = str.trim().toUpperCase();

    if (!/^[A-Z][0-9]{9}$/.test(sanitized)) {
      return false;
    }

    return !!Array.from(sanitized).reduce<number | boolean>((_sum: number | boolean, number: string, index: number) => {
...
export function isIdentityCard(str: unknown, locale: string) {
  assertString(str);
