 ...
 */
function countChars(str: unknown) {
  assertString(str);
...
function scorePassword(analysis: PasswordAnalysis, scoringOptions: PasswordScoreOptions) {
  let points = 0;
...
export function isStrongPassword(str: unknown, options: IsStrongPasswordOptions = {}) {
  assertString(str);
  ...
  if (_options.returnScore) {
    return scorePassword(analysis, _options);
  }
