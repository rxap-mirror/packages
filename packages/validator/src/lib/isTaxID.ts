...
function deDeCheck(tin: any) {
  ...
  occurences = occurences.filter((a: any) => a.length > 1);
  ...
  if (occurences[0].length === 3) {
    const trip_locations = occurences[0].split('').map((a: any) => parseInt(a, 10));
 ...
 */
function dkDkCheck(tin: any) {
  tin = tin.replace(/\W/, '');

  // Extract year, check if valid for given century digit and add century
  let year: string | number = parseInt(tin.slice(4, 6), 10);
 ...
 */
function elCyCheck(tin: any) {
  // split digits into an array for further processing
  const digits = tin.slice(0, 8).split('').map((a: any) => parseInt(a, 10));
 ...
 */
function enIeCheck(tin: any) {
  let checksum = algorithms.reverseMultiplyAndSum(tin.split('').slice(0, 7).map((a: any) => parseInt(a, 10)), 8);
...
function itItCheck(tin: any) {
  ...
  const month_replace: Record<string, string> = {
    A: '01',
    B: '02',
    C: '03',
    D: '04',
  ...
  const month: string = month_replace[chars[8]];
  ...
  const odd_convert: Record<string, number> = { // Maps of characters at odd places
    A: 1,
    B: 0,
    C: 5,
    D: 7,
  ...
  for (let i = 0; i < chars.length - 1; i += 2) {
    ...
    if (chars[i] in odd_convert) {
      ...
      const multiplier = parseInt(chars[i], 10);
...
function svSeCheck(tin: any) {
  ...
  if (tin.length > 11) {
    ...
    if (tin.length === 11 && day < 60) {
      ...
      let current_year: number | string = new Date().getFullYear().toString();
 ...
 */
const taxIdFormat: Record<string, RegExp> = {
...
const taxIdCheck: Record<string, (tin: any) => boolean> = {
...
const allsymbols = /[-\\\/!@#$%\^&\*\(\)\+\=\[\]]+/g;
const sanitizeRegexes: Record<string, RegExp> = {
  'de-AT': allsymbols,
  'de-DE': /[\/\\]/g,
  'fr-BE': allsymbols,
};
 ...
 */
export function isTaxID(str: unknown, locale = 'en-US') {
  assertString(str);
