...
import { createBinaryFromJsonAsYAML } from '../utils/binary';
...
export async function execute(this: IExecuteFunctions, items: INodeExecutionData[]): Promise<INodeExecutionData[]> {
  ...
  if (mode === 'once') {
    ...
    try {
      ...
      const binaryData = await createBinaryFromJsonAsYAML.call(
        this,
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        const binaryData = await createBinaryFromJsonAsYAML.call(this, items[i].json, {
          fileName: options['fileName'] as string,
          encoding: options['encoding'] as string,
          addBOM: options['addBOM'] as boolean,
          format: options['format'] as boolean,
