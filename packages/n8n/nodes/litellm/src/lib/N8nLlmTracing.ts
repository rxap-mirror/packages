...
export class N8nLlmTracing extends BaseCallbackHandler {
  ...
  options = {
    ...
    tokensUsageParser: (llmOutput: LLMResult['llmOutput']) => {
      const completionTokens = (
                                 llmOutput?.['tokenUsage']?.completionTokens as number
                               ) ?? 0;
      const promptTokens = (
                             llmOutput?.['tokenUsage']?.promptTokens as number
                           ) ?? 0;
  ...
  override async handleLLMStart(llm: Serialized, prompts: string[], runId: string) {
