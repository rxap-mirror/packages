...
export class KeycloakService {
   ...
   */
    // TODO : add checks to verify if the instance is initialized before it is used.
  ...
  get enableBearerInterceptor(): boolean {
    return this._enableBearerInterceptor ?? false;
  }
  ...
  public addTokenToHeader(headers: HttpHeaders = new HttpHeaders()) {
    return from(this.getToken()).pipe(
      map((token) =>
        token && this._authorizationHeaderName
          ? headers.set(
  ...
  private bindsKeycloakEvents(): void {
    this._instance.onAuthError = (errorData: any) => {
    ...
    this._instance.onActionUpdate = (state: any) => {
      this._keycloakEvents$.next({
        args: state,
    ...
    this._instance.onReady = (authenticated: boolean) => {
      this._keycloakEvents$.next({
        args: authenticated,
