...
### 1. Relative `require` Failures in Generator Context
...
### 2. Querying `node_modules` and Project Files via Virtual `Tree` Anti-pattern
...
### 3. Dead Code / Inoperative Init Generator due to Missing Peer Dependencies
...
### 4. Lack of Dotted-Path Traversal in `MockConfigServiceFactory`
...
### 5. Tight Coupling to Jest in `MockLoggerFactory`
...
### 6. Missing `fatal` Method on Mock Logger
- **Location**: [packages/nest/testing/src/lib/mock-logger-factory.ts](file:///mnt/mmuenker/Projects/rxap/packages/packages/nest/testing/src/lib/mock-logger-factory.ts)
