...
import {
  ClientRMQExchange,
  RmqExchangeOptions
} from './client-rmq-exchange';

export class ClientRmqExchangeModuleOptionsFactory implements ClientsModuleOptionsFactory {
  ...
  async createClientOptions(): Promise<ClientProvider> {
    ...
    let options: RmqExchangeOptions['options'];
    ...
    if (this.config.get('RABBITMQ_DISABLED')) {
      ...
      options = {
        ...
        exchange: this.config.getOrThrow('GITLAB_EVENTS_EXCHANGE'),
        exchangeOptions: {
          durable: true
        }
      };
