...
export async function initGenerator(tree: Tree, options: InitGeneratorSchema) {
  ...
  for (const peer of Object.keys(missingPeerDependencies)) {
    ...
    const config = JSON.parse(tree.read(join('node_modules', ...peer.split('/'), configFile), 'utf-8')!);
    ...
    if (!tree.exists(fullInitGeneratorFilePath)) {
      console.log(`Peer dependency ${peer} has no init generator file: ` + fullInitGeneratorFilePath);
      continue;
    }
