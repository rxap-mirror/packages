...
describe('SubscriptionHandler', () => {
  ...
  it('reset subscription without key', () => {
    ...
    const unsubscribeSpy = jest.spyOn(sh, 'unsubscribe');

    sh.reset();

    expect(unsubscribeSpy.mock.calls.length).toBe(1);

    expect(unsubscribeSpy.mock.calls[0][0]).toBe(SubscriptionHandler.DEFAULT_KEY);
  ...
  it('reset all', () => {
    ...
    expect(unsubscribeSpy.mock.calls.length).toBe(3);
    expect(unsubscribeSpy.mock.calls[0][0]).toBe(key1);
    expect(unsubscribeSpy.mock.calls[1][0]).toBe(key2);
    expect(unsubscribeSpy.mock.calls[2][0]).toBe(key3);
