...
validationSchema['STATUS_SERVICE_BASE_URL'] = Joi.string().default(environment.production ? 'http://rxap-service-status:3000' : 'http://localhost:5300');
...
validationSchema['GLOBAL_API_PREFIX'] = Joi.string().default('api/app/angular/accordion');
validationSchema['PORT'] = Joi.number().default(3842);
validationSchema['SENTRY_DEBUG'] = Joi.string().default(environment.sentry?.debug ?? false);
validationSchema['SENTRY_SERVER_NAME'] = Joi.string().default(process.env.ROOT_DOMAIN ?? 'service-app-angular-accordion');
