...
while [ "$1" != "" ]; do
  ...
  --without-services) without_services="true" ;;
  --without-frontends) without_frontends="true" ;;
  *)
    echo "Usage: $0 --local"
    exit 1
    ;;
...
LOCAL_DOCKER_COMPOSE_FILES=""
if [[ $without_services == "false" ]]; then
  LOCAL_DOCKER_COMPOSE_FILES="${LOCAL_DOCKER_COMPOSE_FILES} -f docker-compose.services.yml"
fi
if [[ $without_frontends == "false" ]]; then
  LOCAL_DOCKER_COMPOSE_FILES="${LOCAL_DOCKER_COMPOSE_FILES} -f docker-compose.frontends.yml"
fi
