...
export class LanguageSelectorService {
  ...
  private redirect(language: string) {
    ...
    const redirectUrl     = location.origin + `/${language}` + location.pathname.replace(new RegExp(`^\/${this.selectedLanguage}`), '') + location.search;
...
export class LanguageSelectorComponent {
  ...
  constructor(
    public readonly language: LanguageSelectorService
  ) {}
