...
export class TableColumnMenuComponent implements AfterContentInit {
  ...
  public updateDisplayColumns() {
    ...
    this.displayColumns = this.columns.filter(option => option.active).map(option => option.name);
  }

  public activate(columnName: string) {
    this.columns?.filter(option => option.name === columnName).forEach(column => column.activate());
    this.updateDisplayColumns();
  }

  public deactivate(columnName: string) {
    this.columns?.filter(option => option.name === columnName).forEach(column => column.deactivate());
    this.updateDisplayColumns();
  }
