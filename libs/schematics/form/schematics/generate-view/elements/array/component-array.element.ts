...
export class ComponentArrayElement extends ArrayElement {
  ...
  public postParse() {
    if (!this.componentName) {
      ...
      this.from = `./${dasherize(this.name)}-array-group/${dasherize(
        this.name
      )}-array-group.component.module`;
          ...
          return chain([
            ...
            () => {
              const componentSourceFile = project.createSourceFile(
                `./${dasherize(this.name)}-array-group/${dasherize(
                  this.name
                )}-array-group.component.ts`
              );
                      ...
                      Writers.object({
                        ...
                        templateUrl: (w) =>
                          w.quote(
                            `./${dasherize(
                              this.name
                            )}-array-group.component.html`
                          ),
                        styleUrls: `[ './${dasherize(
                          this.name
                        )}-array-group.component.scss' ]`,
                        changeDetection: 'ChangeDetectionStrategy.OnPush',
                        host: Writers.object({
                          class: (w) =>
                            w.quote(`rxap-${dasherize(this.name)}-array-group`),
                        }),
                        providers: (w) => {
                          w.writeLine('[');
                          Writers.object({
