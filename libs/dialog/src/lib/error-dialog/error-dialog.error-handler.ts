  ...
  public handleError(error: any): void {
    ...
    if (error) {
      ...
      if (error instanceof RxapError) {
        rxapError = error;
      } else if ((error.hasOwnProperty('rejection') && error[ 'rejection' ] instanceof RxapError)) {
      ...
      if (rxapError) {
        ...
        if (!this.lastErrorMessage || (this.lastErrorMessage[ 0 ].message !== rxapError.message || this.lastErrorMessage[ 1 ] + 10000 < Date.now())) {

          this.dialog.open(ErrorDialogComponent, { data: { error: rxapError }, hasBackdrop: true });
