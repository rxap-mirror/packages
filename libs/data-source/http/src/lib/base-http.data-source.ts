...
export abstract class BaseHttpDataSource<Data = any, PathParams = any, Body = any> extends BaseDataSource<
  ...
  protected _data$!: ReplaySubject<Data>;
  ...
  public abstract derive(id: string, metadata: Partial<HttpDataSourceMetadata<PathParams, Body>>): BaseHttpDataSource<Data>;

  public request$(options: HttpDataSourceOptions<PathParams, Body> = this._options, merge: boolean = false): Promise<Data> {
    this.init();
    if (merge) {
      // TODO : add deep merge
    ...
    return this.buildRequest(options).toPromise();
  }
  ...
  protected _connect(viewer: HttpDataSourceViewer<PathParams, Body>): [ Observable<Data>, Subscription ] {
    this.init();
